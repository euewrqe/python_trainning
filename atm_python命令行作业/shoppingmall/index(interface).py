...
f.close()
## 整理字典
import core.main as bal_face

salary = bal_face.toBalance(0)
choice_li = []
...
def toBalance():
    ...
    flag = input("是否结算[y/n]")
    if flag:
        salary = bal_face.toBalance(balance_count)
...
def buy_func(pro_dict):
    '''
    如果你要直接购买商品，输入buy可以直接看商品
    :param pro_dict:
    '''
...
def cleanDict(pro_dict):
    ...
    while True:
        ...
        if not buy_flag:
            ...
            # 如果该键的值为None，且该键的格式是:- 女装山脉 & 2000,就打印出来，且加入到menu_key里面
            buy_func(pro_dict)
            for i, k in enumerate(menu_key_li):
                print("%s. %s" % (i, k))
        ...
        if num.isdigit():
            ...
            toBalance()
        elif num == "buy":    # 直接列出购买列表
            buy_flag = True
        elif num == "watch":   # 一层层选购
            buy_flag = False
        elif num == "help":
            print('''
            您可以选择：
            ...
            \033[32mbuy\033[0m:直接查看购买列表
            \033[32mwatch\033[0m:一层层选购
            ''')
        elif num == "list":  # 列出购物车
            for goods, dollar in choice_li:
                print("%s,价格 \033[32m%s\033[0m" % (goods, dollar))
        elif num == "del":   # 列出购物车，选出你要撤回的商品
